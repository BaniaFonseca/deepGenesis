...
class RetinaNet():
    ...
    def fpn(self, inputs):
        ...
        #P5
        m5 = c5
        p5 = Conv2D(filters=32, kernel_size=3, strides=1, padding='same',
                        kernel_initializer='glorot_uniform', use_bias=False)(m5)
        p5 = self.batch_norm(p5)
        p5 = keras.layers.advanced_activations.LeakyReLU(alpha=_LEAKY_RELU, name='P5')(p5)

        up_m5 = UpSampling2D(size=2)(m5)
        cv_c4 = Conv2D(filters=2048, kernel_size=1, strides=1, padding='same',
        ...
        m4 = keras.layers.Add(name='m4')([up_m5, cv_c4])

        p4 = Conv2D(filters=32, kernel_size=3, strides=1, padding='same',
                    kernel_initializer='glorot_uniform', use_bias=False)(m4)
        p4 = self.batch_norm(p4)
        p4 = keras.layers.advanced_activations.LeakyReLU(alpha=_LEAKY_RELU, name='P4')(p4)

        up_m4 = UpSampling2D(size=2)(m4)
        cv_c3 = Conv2D(filters=2048, kernel_size=1, strides=1, padding='same',
                       kernel_initializer='glorot_uniform', use_bias=False)(c3)
        cv_c3 = self.batch_norm(cv_c3)
        cv_c3 = keras.layers.advanced_activations.LeakyReLU(alpha=_LEAKY_RELU)(cv_c3)

        m3 = keras.layers.Add(name='m3')([up_m4, cv_c3])

        p3 = Conv2D(filters=32, kernel_size=3, strides=1, padding='same',
                    kernel_initializer='glorot_uniform', use_bias=False)(m3)
        p3 = self.batch_norm(p3)
        p3 = keras.layers.advanced_activations.LeakyReLU(alpha=_LEAKY_RELU, name='P3')(p3)
        return p3, p4, p5
    ...
    def conv_II(self, inputs):
        ...
        for _ in range(3):
            ...
            shortcut = Conv2D(filters=256, kernel_size=3, strides=1, padding='SAME',
    ...
    def conv_III(self, inputs):
        ...
        for i in range(2):
            ...
            inputs = keras.layers.Add()([inputs, shortcut])
            inputs = keras.layers.advanced_activations.LeakyReLU(alpha=_LEAKY_RELU,
                                                    name='CONV_3_'+str(i))(inputs)
            shortcut = inputs
    ...
    def conv_IV(self, inputs):
        ...
        for i in range(1, 2 + 1):
            # 1
            inputs = Conv2D(filters=256, kernel_size=1, strides=self.get_strides(i),
                            padding='SAME', kernel_initializer='glorot_uniform', use_bias=False)(inputs)
            inputs = self.batch_norm(inputs)
            inputs = keras.layers.advanced_activations.LeakyReLU(alpha=_LEAKY_RELU)(inputs)
            # 2
            ...
            # adjust shape with convolution
            shortcut = Conv2D(filters=256, kernel_size=1, strides=self.get_strides(i), padding='SAME',
                              kernel_initializer='glorot_uniform', use_bias=False)(shortcut)
            shortcut = self.batch_norm(shortcut)
    ...
    def conv_V(self, inputs):
        ...
        for i in range(1, 2 + 1):
            ...
            shortcut = Conv2D(filters=512, kernel_size=1, strides=self.get_strides(i), padding='SAME',
                              kernel_initializer='glorot_uniform', use_bias=False)(shortcut)
            shortcut = self.batch_norm(shortcut)
            shortcut = Conv2D(filters=2048, kernel_size=3, strides=1,
                              padding='SAME', kernel_initializer='glorot_uniform', use_bias=False)(shortcut)
            shortcut = self.batch_norm(shortcut)
